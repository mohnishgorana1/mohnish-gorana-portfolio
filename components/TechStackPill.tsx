...
function TechStackPill({ tech, isBlackIcon, Icon, techData }: { tech: string, isBlackIcon: boolean, Icon: React.ElementType, techData: { color: string } }) {
    return (
        <button
            key={tech}
            type="button"
            className={cn("group/pill flex items-center justify-center gap-2 px-4 py-1.5 rounded-lg cursor-pointer",
                "bg-secondary/30",
                "border border-border dark:border-border group-hover/pill:border-border ",
            ...
            <div
                className={`flex items-center justify-center ${isBlackIcon
                    ? "dark:invert opacity-80 group-hover/pill:opacity-100"
                    : "opacity-90 group-hover/pill:opacity-100"
                    } transition-opacity`}
            >
                <Icon size={14} color={isBlackIcon ? "#000000" : techData.color} />
            </div>
            <span className="font-semibold text-foreground/80 group-hover/pill:text-foreground text-xs transition-colors tracking-tight">
                {tech}
            </span>
        </button>
    );
